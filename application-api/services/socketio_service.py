...
def setup(sio: socketio.AsyncServer):
    ...
    async def authenticate(sid, token):
        try:
            ...
            async with sio.session(sid) as session:
                ...
                await sio.emit(
                    "user_activities_update", user_update_payload, skip_sid=sid
                )
    ...
    async def upload_document(sid, room_id, json_data, filename):
        try:
            ...
            check_user_exists_in_room(
                sid, "upload_document_response", username, room_id
            )
    ...
    async def test_download(sid, _):
        ...
        throughput_kbps = (packets_sent * 1024) / (
            end_time - start_time
        ).total_seconds()
